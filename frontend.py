...
st.caption("Past briefs are in the left sidebar. Use the chat bar at the bottom to generate a new brief.")
...
with st.sidebar:
    ...
    if not past_files:
        st.caption("No saved briefs found (*.md in current folder).")
        ...
        if st.button("📂 Load selected brief"):
            if selected_md_file:
...
incoming_topic = st.chat_input("Type launch context and press Enter to generate")
...
tab_plan, tab_evidence, tab_preview, tab_images, tab_logs = st.tabs(
    ["🧭 Strategy Plan", "🔎 Market Evidence", "📝 Brief Preview", "🖼️ Visuals", "🧾 Logs"]
)
...
if run_btn:
    if not topic:
        st.warning("Please enter launch context.")
        st.stop()
...
if out:
    ...
    with tab_plan:
        ...
        if not plan_obj:
            ...
            st.write("**Brief Title:**", plan_dict.get("blog_title"))
            cols = st.columns(3)
            cols[0].write("**Audience:** " + str(plan_dict.get("audience")))
            cols[1].write("**Tone:** " + str(plan_dict.get("tone")))
            cols[2].write("**Brief type:** " + str(plan_dict.get("blog_kind", "")))
    ...
    # --- Preview tab ---
    with tab_preview:
        st.subheader("Launch Brief Preview")
        ...
        if not final_md:
            ...
            if hasattr(plan_obj, "blog_title"):
                ...
                blog_title = plan_obj.get("blog_title", "launch_brief")
            else:
                # fallback: parse from markdown title
                blog_title = extract_title_from_md(final_md, "launch_brief")
    ...
    with tab_images:
        ...
        if not specs and not images_dir.exists():
            st.info("No visuals generated for this brief.")
        else:
            if specs:
                st.write("**Image plan:**")
                st.json(specs)
    ...
    st.info("Enter launch context and click **Generate Brief**.")
